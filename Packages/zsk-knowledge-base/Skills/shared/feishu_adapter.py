...
class FeishuAdapter:
    ...
    def store_original(self, binding: Binding, source: SourceRecord, payload: bytes) -> AdapterResult:
        ...
        return failure or storage.store_original(source, payload)
    def store_readable(self, binding: Binding, source: SourceRecord, payload: bytes) -> AdapterResult:
        storage, failure = self._stage5_source_storage(binding, source)
        return failure or storage.store_readable(source, payload)
    ...
    def write_knowledge_asset(self, binding: Binding, asset: AssetPayload) -> AdapterResult:
        ...
        body = self._visible_asset_body(asset.body)
    ...
    def write_profile(self, binding: Binding, asset: AssetPayload) -> AdapterResult:
        ...
        return source if source.status not in {"ok", "reused"} else self._remember_asset(storage.store_document(f"profile:{asset.asset_id}", asset.title, storage.root_nodes["05"], self._visible_asset_body(asset.body), "profile", "feishu://05"))
    ...
    def read_back(self, binding: Binding, refs: Sequence[BackendObjectRef] | None = None) -> AdapterResult:
        ...
        current = {ref.object_id: ref for ref in inspected.object_refs} | self._asset_refs
        ...
        checks = ("objects_present", "binding_identity", "stable_refs", "docx_content_fingerprints") if all(ref.object_id not in self._asset_refs for ref in wanted) else ("asset_write_readback", "stable_refs")
