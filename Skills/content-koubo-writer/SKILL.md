---
name: content-koubo-writer
description: Content 口播 Slim 的统一口播正文 Writer。用于同一 Run 已生成唯一 Content Context Pack 后，按 Gate A 冻结的 ganhuo、huati 或 zhuanhua 单一主模式生成可直接朗读的正文，或根据真人意见修改当前正文。不得搜索 Vault、重选模式、调用旧 Writer 或 Reviewer、生成标题配套、保存或进入 P5。
---
...
无外部参考时，外部蓝图为空不代表缺 Context；使用同一 Pack 的 `approved_direction.structure_plan` 和已选 04 的具体 `writer_context`。同行贡献内容价值，方法贡献组织办法，03/05 贡献获准事实。客户给出的操作或风格要求只执行，不写成正文；不得把“先找同行再套结构”等内部说明念给观众。
...
- 04 的 `source_metadata` 是不可丢弃的使用限制；历史、仅方法参考或仍待原文核验的内容不成为当前事实，也不能因为进入 Context 就视为已核验；
...
设计原因保存在 Factory 设计卡；版本、状态和确认由 `content-koubo-slim` Runtime 负责。不要调用三个旧 Writer Skill。

## 单篇交付检查

每次只写 Context Pack 对应的一篇可独立录制的正文。批次中的其他篇由入口分别调度；若收到“在同一正文放10版”等批量执行要求，不把它当必须写进正文的内容，应退回入口拆分任务。不要擅自删掉数量要求只交一篇。

提交前检查：正文只有本篇内容；没有“版本A/脚本2”等多稿包装；开场、推进和结尾属于同一条口播；没有内部思考、工具说明或过程标签。正文中的正常步骤、数字和对软件版本的讨论可保留，不能为过格式检查删除用户实质观点。只返回正文合同，单稿的修订版本由程序记录。
