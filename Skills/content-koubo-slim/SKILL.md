...
description: Content 口播 Slim 的唯一用户入口。使用已连接的 Obsidian 或飞书知识库，开始、继续、恢复或查看单篇或多篇短视频口播任务，每篇以独立 Run 协调方向、正文、抖音/小红书/视频号配套三次真人确认；支持保存后仅修订配套并保留旧版，产物始终未发布。仅做编排；不得亲自写稿、发布或调用旧 Content V1 Skill。
---
...
用一个入口管理单篇或多篇 Content 口播 Slim 任务，每篇独立推进；P5 为已确认正文生成抖音、小红书、视频号各一套标题、发布正文和 Tag，经同一个第三真人停点后安全保存口播与配套，然后保持未发布并停下。
...
- 保存后只修订一个或多个平台配套时，复用已核验口播并另存新配套版本，不重写或重批正文；
...
15. P5 重新校验完整已确认正文，并从已批准方向与唯一 Context 确定性投影受众、内容目的、讲述模式、实际存在的口吻样例和 `must_avoid`；它们只影响写法，正文仍是事实与承诺唯一边界。目标平台取本次明确要求或当前已确认默认，缺省为抖音、小红书、视频号，不为每篇重复确认；
16. `$content-koubo-publish-pack` 按可携带平台规则快照生成每个平台一套可直接使用的标题、发布正文和 1—5 个相关 Tag，可选 0—2 条封面文字。程序记录平台集合、规则版本、正文绑定和写作条件哈希，不重搜 03/04/05；
17. 用户选择“需要修改”时保留旧版本。明确只改某平台字段时传 `平台.字段` scope；程序把基版、正文、目标平台、规则、反馈和 scope 持久绑定到下一配套版本，跨进程记录结果时即使省略参数也不得扩大范围。整套修改或增减平台必须明确传目标集合；
18. 用户选择“确认并保存”或“三个平台都采用推荐项”时，程序固定绑定当前完整配套并立即保存，不增加第四个确认点；v2 每个平台已是一套成品，旧 v1 仍保留原候选标题选择语义；
19. 程序重新校验 Registry、Manifest、Profile 索引及 P3 实际读取的 03/04/05 哈希，再从 Manifest 推导输出根。首次仍 create-only 写入并回读“纯口播稿 + 三平台配套”两份文件；归档名按本篇冻结主题稳定生成，不随平台标题变化；
20. 保存后仅修订配套时，回读核验匹配的原口播回执，仅 create-only 新配套文档。回执按正文版本与配套版本双键隔离，本地与飞书失败均复用同一请求恢复，旧配套不覆盖；
21. 初次两份文件都成功、或配套修订完成且原口播仍匹配后才进入 `saved`；始终保持 `publish_status=not_requested`，P5 停止，不进入 P6。

## 真人停点

完整产品只有三个真人停点：方向、正文、三平台配套与保存。P5 必须完整展示所有目标平台，再接受“确认并保存”（“三个平台都采用推荐项”映射为同一决定）或带具体意见的“需要修改”；“只改小红书正文”等明确局部意见必须设置对应 scope，未指定平台保持原文。确认动作立即保存，不增加第四次询问。
...
- 正文未确认、目标平台/配套字段不合法、正文绑定或确认版本不一致、局部修改越界时停止；
- Registry 重绑、Manifest 无效、输出路径越界、目标重名或双文件任一写入/回读失败时，不报告保存完成；
- 保存阶段重试只复用已确认正文与当前配套，不重新生成内容；保存后仅改配套时复用内容匹配的已保存口播，不复用旧配套回执；
...
- 配套 Skill 不读取 Vault、不修改或重复输出已确认正文；平台规则是独立发行快照，不依赖另一台机器上的 `publish-copy`；
- P5 只写 Manifest 授权输出根：首次为两份最终产物，保存后配套修订只新增一份配套并核验原口播；不修改客户 01—05。飞书只保存到授权的 07 下，所有本次所需文档回读匹配才完成；
- 不调用旧标题、分发或保存 Skill，不增加 Reviewer、状态、发布能力或 P6 产物；
- 不修改客户 RC、Host、main、Tag，不执行客户内容的 Git、推送、同步、上传或发布。

## 最小示例

正文 `draft_v2` 已确认后，P5 生成抖音、小红书、视频号三套成品并展示为 `package_v1`。用户说“只改小红书正文”时生成 `package_v2`，抖音、视频号和封面文字逐值保留。若 `package_v1` 已保存，程序核验并复用原口播，只另存 `package_v2` 配套；旧文件全部保留。
...
- 三平台发布正文按各平台编辑规则和本次明确长度/格式要求组织；编辑建议不是平台硬限制，也不要把配文字数解释成口播时长。用户要求口播时长时，在正文确认前给出字数和按实际语速估算的时长区间，不能承诺精确秒数。
- 一篇口播须有自然收尾。普通观点总结或与主题有关的评论交流不应被商业转化规则一律禁掉；按用户当次要求及已有项目偏好执行。引流私信、领取、预约、购买、免费和名额等仍须有相应事实与授权，不由程序自造。

配套结果提交必须使用 `prepare-package` 冻结的正文版本和生成请求。`record-package` 可省略已经持久化的 `target_platforms`、反馈和 `revision_scope`，但显式传入时必须逐值一致，不能改填版本或扩大修改范围。正文已修改后，旧配套结果必须丢弃并重新生成；修改请求产生后，没有新 `package_vN` 不能再次确认旧配套。
