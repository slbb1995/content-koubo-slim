...
17. 用户选择“需要修改”时保留旧版本。明确只改某平台字段时传 `平台.字段` scope，程序逐值保护其他平台和封面内容；整套修改或增减平台必须明确传目标集合；
...
配套结果提交必须把 `prepare-package` 返回的 `approved_draft.draft_version`、`target_platforms` 和 `revision_scope` 原样传回 `record-package`；不得在提交时改填当前版本或扩大修改范围。正文已修改后，旧配套结果必须丢弃并重新生成。
