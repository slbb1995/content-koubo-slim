---
name: content-context-retriever
description: Content V2 Slim 的 Gate A 后客户上下文装配 Skill。用于同一 Run 已批准方向并冻结 04、business_context_needs、speaker_mode 与 Writer 模式后，复用已选 04、按需筛选 03、在 personal_ip 模式读取唯一主 Profile，生成一份 Content Context Pack。不得重搜 04、改变方向、调用 Writer、写正文或进入 P4。
---
...
设计原因保存在 Factory 设计卡；确定性实现位于 `content-slim/runtime/`。不要把 Runtime 或完整 Schema 复制回本文件。
