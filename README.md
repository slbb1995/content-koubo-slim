...
它从明确或已确认默认的 Obsidian 或飞书知识库中选择本次 IP，把客户的一段想法结合少量内容资料写成口播，依次完成方向、正文、三平台配套文案三次真人确认。每篇为抖音、小红书、视频号各生成一套标题、发布正文和 Tag，首次保存口播与配套两份产物；保存后可只修订配套并保留旧版。全部结果保持未发布。
...
- **content-koubo-publish-pack**：正文确认后为抖音、小红书、视频号生成各自的标题、发布正文和 Tag。
...
| 版本 | Skill 数量 | 功能 |
|---|---:|---|
| 1.4.0 | 5 | 三平台配套、平台局部修订、正文/配套双版本保存恢复 |
...
1. 完整展示方向后停下；
2. 完整展示正文后停下；
3. 完整展示抖音、小红书、视频号三套配套文案后停下。

只有我明确确认后才能进入下一步。
首次最终确认后保存两份 Markdown；保存后只改配套时复用原口播并新增配套版本，保持未发布。
~~~

## 三平台配套与保存后修订

P5 默认目标平台为抖音、小红书、视频号，每个平台输出一条成品标题、一段发布正文和 1—5 个相关 Tag；封面文字可选。平台差异只改变切入重点和信息组织，客户事实、身份、案例、产品能力、承诺及 CTA 仍以已确认口播正文为上限。

目标平台也可以由本次请求明确指定。第一次确定后写入当前配套版本；不为批次中的每篇重复增加确认。用户明确“只改小红书正文”时，入口使用 `--revision-scope xiaohongshu.publish_copy`，生成请求会随 Run 持久化，跨 CLI 或漏传重复参数也不能顺带修改其他平台。用户说“三个平台都采用推荐项”会由 Runtime 精确映射为“确认并保存”；相近的模糊表述不算确认，也不增加第四次保存确认。

新配套合同与旧通用配套并存：旧 Run、旧批准记录和旧回执继续按 v1 读取和恢复，不自动补三平台。新合同保存回执同时绑定正文版本与配套版本。归档名来自本篇冻结主题，不随某个平台标题改变；同名批次仍由批次条目后缀隔离。
...
内部接口为 `discover-methods` → 完整回读候选 → `start --method-selection`。选择文件只供 Agent 使用，详见 `Skills/content-koubo-slim/references/idea-first-materials.md`。旧 CLI 不传选择文件仍保留字面检索兼容；旧 Run、三次真人确认、唯一 Context 和首次双文件未发布保存保持兼容。
...
其中需要实际覆盖：五个 Skill 和发行清单一致；新旧配套合同均可读；三平台字段与正文绑定有效；平台局部修改不会改变其他平台；保存后配套修订复用已核验口播；本地与飞书的回读、已确认失败重试和未知结果停止；旧 v1 Run 与回执恢复；同名批次不串稿；三行业、三种内容目的及三种讲述模式的合成样稿不串客户。格式通过不能替代合成样稿的人工内容检查，合成结果也不能表述为真实流量验证。
