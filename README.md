...
目标平台也可以由本次请求明确指定。第一次确定后写入当前配套版本；不为批次中的每篇重复增加确认。用户明确“只改小红书正文”时，入口使用 `--revision-scope xiaohongshu.publish_copy`，程序会拒绝其他平台被顺带改动。用户说“三个平台都采用推荐项”按现有第三个真人停点处理，不增加第四次保存确认。
