...
from scripts.content_slim import build_parser, prepare_direction_stage  # noqa: E402
...
class DefaultClientTests(unittest.TestCase):
    def temporary_root(self):
        ...
        return tempfile.TemporaryDirectory(prefix="content-default-client-", dir=parent)
    ...
    def test_default_paths_follow_current_codex_home(self) -> None:
        with self.temporary_root() as directory:
            ...
            with mock.patch.dict(os.environ, {"CODEX_HOME": str(host)}):
                self.assertEqual(
                    default_registry_path(),
                    host / ".content-v2-slim" / "client-registry.json",
                )
                self.assertEqual(
                    default_runs_root(),
                    host / ".content-v2-slim" / "runs",
                )
